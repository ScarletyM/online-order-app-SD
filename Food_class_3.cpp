...
class Quesadilla: public Food {
    ...
    bool is_half;
//constructor
Quesadilla(float p, char *n, int i, int c, bool a, int t, bool h, char *top): Food(p, n, i, c, a, t){
    is_half = h;
    toppings_size = 0;
    while(top[toppings_size] != 0) {toppings_size++;}
    toppings_size++;
...
// copy constructor 
    Quesadilla(const Quesadilla &q):Food(q) {
        is_half = q.is_half;
        toppings_size = q.toppings_size;
        toppings = new char[toppings_size];
        for(int i = 0; i < toppings_size; i++) {
            toppings[i] = q.toppings[i];
        }
...
// accesors
    void display_quesidilla() {
        cout << toppings << endl;
        cout << is_half << endl;
    }

};
// this class seems redundant as has almost all the same atributes as the quesidilla classs
//class Shake: public Food {
//public:
    //bool is_big;
    //char *flavor;
