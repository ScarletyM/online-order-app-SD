...
void display_orderhis_unfinished(bool nightmode, int num_order) {
  if (nightmode==0) {
...
void display_orderhis_finished(bool nightmode, int num_order) {
  if (nightmode==0) {
    add_yaml("order_items.yaml",{{"tc","black"},{"bgdc","\"#b5b5b5\""},{"ordernum",num_order},{"time_index",7000+num_order*200},{"quant_index",7000+num_order*200+20},{"totp_index",7000+num_order*200+23}});
  } else {
    add_yaml("order_items.yaml",{{"tc","white"},{"bgdc","\"#b5b5b5\""},{"ordernum",num_order},{"time_index",7000+num_order*200},{"quant_index",7000+num_order*200+20},{"totp_index",7000+num_order*200+23}});
  }
}
void display_orders_unfinished(bool nightmode, int num_order) {
  if (nightmode==0) {
...
void display_orders_finished(bool nightmode, int num_order) {
  if (nightmode==0) {
    add_yaml("order_items_worker.yaml",{{"tc","black"},{"bgdc","\"#b5b5b5\""},{"boc","black"},{"ordernum",num_order},{"time_index",7000+num_order*200},{"delivery_index",7000+num_order*200+32},{"payment_index",7000+num_order*200+40},{"phone_num_index",7000+num_order*200+51},{"totp_index",7000+num_order*200+20}});
  } else {
    add_yaml("order_items_worker.yaml",{{"tc","white"},{"bgdc","\"#b5b5b5\""},{"boc","white"},{"ordernum",num_order},{"time_index",7000+num_order*200},{"delivery_index",7000+num_order*200+32},{"payment_index",7000+num_order*200+40},{"phone_num_index",7000+num_order*200+51},{"totp_index",7000+num_order*200+20}});
  }
}
...
void readfile() {
  ...
  f.close();
  f.open("shareinfo.txt",ios::in);
  while (getline(f,line)) {
    stringstream geek(line);
    int lineint = 0;
    geek>>lineint;
    put_int_at(9900+i*5,lineint);
...
//interactions:
  if (just_starting()) {
    ...
    for (int i = 0; i<20; i++) {
      finished.push_back(get_int_at(9900+i*5));
    }
//handling id's 
    for (int i=1; i<9; i++) {
      if (event_id_is("p",i)) {
    ...
    if (global_mem[3]==1) {
      for (int i=0; i<worker_num_of_orders; i++) { //"delete" order
           if (event_id_is("workerorder", i)) {
              global_mem[1] = 'i';
              page = global_mem[1];
              orderid = i;
           }
            if (event_id_is("removeorder", i)) {
              put_int_at(9900+i*5,i);
              fstream f;
              f.open("shareinfo.txt",ios::out|ios::app);
              f<<i;f<<'\n';
              f.close();
              page = global_mem[1];
            }
...
//adding yamls
  ...
  if (1 == page) {
    ...
    if (login == 0) {
      ...
      if (nightmode==0) add_yaml("orderplaced.yaml",{{"tc","black"},{"bgdc","white"}});
    ...
    for (int i = 0; i<num_of_orders; i++) {
      bool found = 0;
      for (int j=0; j<20; j++) {
        if (get_int_at(9900+j*5) == i) {
          display_orderhis_finished(nightmode, i);
          found = 1;
        }
      }
      if (found==0) display_orderhis_unfinished(nightmode, i);
    }
  } else if('c'==page) {
    ...
    for (int i = 0; i<worker_num_of_orders; i++) {
      ...
      for (int j=0; j<20; j++) {
        if (get_int_at(9900+j*5) == i) {
          display_orders_finished(nightmode, i);
          found = 1;
        }
      }
      if (found==0) display_orders_unfinished(nightmode, i);
    }
  } else if('i'==page) {
    for (int n = 0; n<20; n++) {
